(() => {
  ...
  async function run(){
    ...
    const labels=load();let okHit=0,fallbackHit=0,checked=0;
    for(const tr of rows){
      ...
      try{
        ...
        if(ok?.found&&ok?.exchange){const tw=/^(maicoin|max|bitopro)$/i.test(ok.exchange);const lab={exchange:ok.exchange,region:tw?'台灣':'海外',tags:'OKLink 明確交易所名稱',source:ok.source||'OKLink'};labels[cp]=lab;save(labels);paint(cell,lab);okHit++;await sleep(120);continue;}
        const fb=labels[cp]?{found:true,label:labels[cp]}:await fallbackLookup(cp,chainId);
    ...
    const head=box.firstElementChild;if(head){const suffix=`｜OKLink 查 ${checked} 個地址｜命中交易所 ${okHit} 筆${fallbackHit?`｜既有地址庫 ${fallbackHit} 筆`:''}`;head.textContent=head.textContent.replace(/｜OKLink 查.*$/,'').replace(/｜地址標籤.*$/,'').replace(/｜自動(?:標記|辨識).*$/,'')+suffix;}
