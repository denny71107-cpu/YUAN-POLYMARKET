(() => {
      ...
      for (const req of requests) {
        ...
        if (known.length) {
          for (const address of known) {
            ...
            records.push({ relay: rid, exchange: { exchange: lab.exchange || '未知', region: lab.region || '', tags: lab.tags || '', address }, direction: 'Relay → 對手', amount: req?.amount || req?.data?.amount || '', time, updatedAt: time });
            hits++;
          }
        } else {
  ...
  function install() {
    ...
    if (wrap && !document.getElementById('yeTestWallet')) {
      ...
      test.title = `直接經 Vercel /api/relay 測試 ${DEFAULT_WALLET}`;
      test.onclick = () => testWallet(DEFAULT_WALLET, test).catch(() => {});
