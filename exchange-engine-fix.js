(() => {
  ...
  function pickTransferCounterparty(tx,wallet){
    const transfers=Array.isArray(tx?.transfers)?tx.transfers:[];
    if(!transfers.length)return null;
    const good=transfers.filter(t=>t?.from&&t?.to&&t.from!==ZERO&&t.to!==ZERO);
    if(!good.length)return null;
    const direct=good.find(t=>t.from===wallet||t.to===wallet);
    if(direct)return {cp:direct.from===wallet?direct.to:direct.from,transfer:direct,direction:direct.from===wallet?'Wallet → 對手':'對手 → Wallet'};
    if(tx?.side==='destination'){
      const t=good[good.length-1]; return {cp:t.to,transfer:t,direction:'Relay → 實際收款地址'};
    }
    const t=good[0]; return {cp:t.from,transfer:t,direction:'實際付款地址 → Relay'};
  }
  function renderResolved(requests,labels,wallet){
    const box=ensureTxPanel();if(!box)return{txCount:0,knownCount:0};
    const unique=new Map();
    for(const req of requests){const rid=req?.id||req?.requestId||req?.data?.id||'';for(const tx of(Array.isArray(req?.resolvedTxs)?req.resolvedTxs:[])){const hash=String(tx?.hash||'').toLowerCase(),chainId=tx?.chainId||'',from=String(tx?.from||'').toLowerCase(),to=String(tx?.to||'').toLowerCase(),key=`${chainId}:${hash||rid}`;if(!unique.has(key))unique.set(key,{rid,hash:tx?.hash||'',chainId,side:tx?.side||'',from,to,transfers:Array.isArray(tx?.transfers)?tx.transfers:[]});}}
    const rows=[];let knownCount=0;
    for(const r of unique.values()){
      const picked=pickTransferCounterparty(r,wallet);
      let cp=picked?.cp||'',direction=picked?.direction||'',token=picked?.transfer?.token||'',source='Token Transfer';
      if(!cp){const fromLab=labels[r.from]||null,toLab=labels[r.to]||null;if(fromLab&&!toLab){cp=r.from;direction='交易所 → Relay';}else if(toLab&&!fromLab){cp=r.to;direction='Relay → 交易所';}else if(r.from===wallet){cp=r.to;direction='Wallet → 對手';}else if(r.to===wallet){cp=r.from;direction='對手 → Wallet';}else{cp=r.side==='destination'?r.to:r.from;direction=r.side==='destination'?'Relay → 對手':'對手 → Relay';}source='Tx From/To';}
      const lab=labels[cp]||null;if(lab)knownCount++;
      rows.push({...r,cp,lab,direction,token,source});
    }
    if(!rows.length){box.innerHTML='<div style="padding:10px;color:#b45309;font-size:12px">Relay Request 有資料，但目前後端尚未解析出鏈上交易。</div>';return{txCount:0,knownCount:0};}
    const html=rows.map(r=>{const url=chainExplorer(r.chainId,r.hash),label=r.lab?`${r.lab.exchange||'未知'}${r.lab.region?' / '+r.lab.region:''}`:'待標記';return `<tr><td>${short(r.rid)}</td><td>${chainName(r.chainId)}</td><td>${r.side||'—'}</td><td>${url?`<a target="_blank" href="${url}">${short(r.hash)}</a>`:short(r.hash)}</td><td>${r.source}</td><td title="${r.token}">${short(r.token)}</td><td>${r.direction}</td><td title="${r.cp}">${short(r.cp)}</td><td style="font-weight:700;color:${r.lab?'#166534':'#b45309'}">${label}</td></tr>`;}).join('');
    box.innerHTML=`<div style="padding:9px 10px;font-weight:700">Relay 鏈上交易明細：${rows.length} 筆（同 Tx 已去重）｜實際對手優先取 Token Transfer｜已知交易所 ${knownCount} 筆</div><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="background:#edf2f7"><th>Relay</th><th>Chain</th><th>Side</th><th>TxHash</th><th>對手來源</th><th>Token</th><th>資金方向</th><th>實際對手地址</th><th>交易所標籤</th></tr></thead><tbody>${html}</tbody></table>`;
  ...
  async function testWallet(wallet,testButton){const w=String(wallet||'').trim().toLowerCase();if(!validWallet(w))throw new Error('Wallet 格式不正確。');const p=document.getElementById('yeProgress');if(p)p.textContent=`正在透過 Vercel 查 Relay、交易收據與 Token Transfer：${w}…`;if(testButton)testButton.disabled=true;try{const url=`/api/relay?wallet=${encodeURIComponent(w)}&limit=50&includeChildRequests=true&enrich=true&_=${Date.now()}`,r=await fetch(url,{method:'GET',cache:'no-store'}),text=await r.text();let data;try{data=JSON.parse(text);}catch{throw new Error(`後端回傳非 JSON（HTTP ${r.status}）：${text.slice(0,300)}`);}if(!r.ok)throw new Error(`Vercel /api/relay HTTP ${r.status}: ${data.detail||data.error||text.slice(0,300)}`);const requests=requestArray(data),labels=labelsFromStorage(),records=[];let hits=0;for(const req of requests){const rid=req?.id||req?.requestId||req?.data?.id||'',time=req?.updatedAt||req?.createdAt||req?.data?.updatedAt||'',known=[...new Set(collectKnownAddresses(req,labels))].filter(a=>a!==w);if(known.length){for(const address of known){const lab=labels[address];records.push({relay:rid,exchange:{exchange:lab.exchange||'未知',region:lab.region||'',tags:lab.tags||'',address},direction:'Relay → 對手',amount:req?.amount||req?.data?.amount||'',time,updatedAt:time});hits++;}}else records.push({relay:rid,exchange:null,exchangeAddress:'',direction:'',amount:req?.amount||req?.data?.amount||'',time,updatedAt:time});}const detail=renderResolved(requests,labels,w),cache=load();cache[w]={wallet:w,records,requests:requests.length,updatedAt:new Date().toISOString()};save(cache);window.__YUAN_CURRENT_ROWS__=[{'Proxy Wallet':w,'帳號名稱':w}];if(typeof window.__YUAN_EXCHANGE_RENDER__==='function')window.__YUAN_EXCHANGE_RENDER__();if(p)p.textContent=`完成：Relay ${requests.length} 筆 Request｜解析 ${detail.txCount} 筆去重鏈上交易｜實際對手以 Token Transfer 優先｜交易所標籤 ${detail.knownCount} 筆｜Wallet ${w}`;return{requests:requests.length,hits,...detail};}catch(e){console.error('YUAN direct Relay test error',e);if(p)p.textContent=`測試失敗：${e?.message||e}`;alert(`測試失敗\n${e?.message||e}`);throw e;}finally{if(testButton)testButton.disabled=false;}}
